...
def FL_Train(init_global_model, client_data_loaders, test_loader, FL_params):
   ...
   #IMPORTANT：因此，为了实现Federated unlearning，我们需要在global train之前就将client——models中的模型进行保存。可以使用deepcopy，或者硬盘io方式。
    ...
    #Therefore, in order to implement Federated Unlearning, we need to save the models in Client -- Models before global Train.You can use DeepCopy, or hard disk IO.
        ...
        all_client_models += client_models ## whats the point of this?? Since client models is passed into fedav. Maybe its to save all the models (as said on line 59)
        ## eitherway shouldnt this be a deepcopy and not +=?
        global_model = fedavg(client_models)
        # print(30*'^')
...
def global_train_once(global_model, client_data_loaders, test_loader, FL_params):
    ...
    #Using the model, optimizer, and data of each client, training the initial model with client_models,
    #updating the UPODate -- client_models using the client user's local data and optimizer
    ...
    for client_idx in range(FL_params.N_client):
        ## If the client_index is line with the one we want to forget, dont do anything
        ...
        #local training, ## Think this is only local as this method does 1 global round
       ...
       ## model.to(device_cpu)
       ## setting the model to the trained model (looped for every client index)
        client_models[client_idx] = model
        
        ## I thought it was "forgotten above by skipping it??"
...
# def fedavg(local_models, local_model_weights=None):
    ...
    ## list of local_state_dicts (for each local model)
    for model in local_models:
        local_state_dicts.append(model.state_dict())
    
    ## for each key, set to 0 and update using the local state dicts

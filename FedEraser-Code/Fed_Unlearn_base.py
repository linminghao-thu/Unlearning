...
def federated_learning_unlearning(init_global_model, client_loaders, test_loader, FL_params):
    ...
    ## old_GMs - global models obtained using FedAvg algorithm
    ## old_CMS - old client models (trained once)
    ...
    ## why is this param set here?? (not in the main?)
...
def unlearning(old_GMs, old_CMs, client_data_loaders, test_loader, FL_params):
    ...
    ## ?? same as unlearn without calib, don't know why its appended 
    ...
    ## Don't know why [-global_epoch] is done here
    ...
    ## Don't know why this is subtracted by 1
    CM_intv  = GM_intv -1
    CM_intv = CM_intv[1:]
    ...
    """
    Since the inIT_model does not contain any information about the forgotten user at the start of the FL training, 
    you just need to overlay the local Model of the other retained users, You can get the Global Model after 
    the first round of global training.
    """
    epoch = 0
    ...
    # Appending the new global model to our list of global models.
    ...
    """
    In this step, the global Model obtained from the first round of global training was used as 
    the new starting point for training, and a small amount of training was carried out with the 
    data of the reserved user (a small amount means reducing the local epoch, i.e. Reduce the number 
    of local training rounds for each user. The parameter forget_local_epoch_ratio is to control 
    and reduce the number of local training rounds.) Gets the direction of iteration of the 
    local Model parameter for each reserved user, starting with new_global_model. Note that 
    this part of the user model is ref_client_models.
    
    Then we use the old_client_models and old_global_models saved from the unforgotten FL training,
    and the ref_client_models and new_global_Model that we get when we forget a user,To build the
    global model for the next round
    
    
    (ref_client_models - new_global_model) / ||ref_client_models - new_global_model||，
    Indicates the direction of model parameter iteration starting with a new global model that 
    removes a user. Mark the direction as step_direction

    ||old_client_models - old_global_model||，Indicates the step size of the model parameter iteration 
    starting with the old global model with a user removed.Step step_length
    
    So, the final direction of the new reference model is 
    step_direction * step_length + new_global_model。
    """
    """
    Intuitive explanation of this part: Usually in IID data, after the data is sharded, 
    the direction of model parameter iteration is roughly the same.
    The basic idea is to take full advantage of the client-model parameter data saved in standard FL
    training, and then, by correcting this part of the parameter, apply it to the iteration of 
    the new global model that forgets a user.
    ...
    ## Changing the local epoch based on the given ratio
...
def unlearning_without_cali(old_global_models, old_client_models, FL_params):
    ...
    if(not(FL_params.forget_client_idx in range(FL_params.N_client))):
        raise ValueError('FL_params.forget_client_idx is note assined correctly, forget_client_idx should in {}'.format(range(FL_params.N_client)))
    ## Client to be forgtten
    forget_client = FL_params.forget_client_idx
    
    ## Are old_client models indexed increasing with global epochs too?
    for ii in range(FL_params.global_epoch): 
        ##???
        temp = old_client_models[ii*FL_params.N_client : ii*FL_params.N_client+FL_params.N_client]
        ## forgets the client indexed to be forgetten
        ## this index should be changed based on the global epoch? (think it is actually cause temp always has same shape)
        temp.pop(forget_client)
        ## Why isn't this appended to a new thing?
        old_client_models.append(temp)
    ...
    ## Making a copy of the inputted global model
    uncali_global_models.append(copy.deepcopy(old_global_models[0]))
    epoch = 0

    # old_client models do not have the forgotten idxs
    ## Why only for seemingly one [epoch]
    ...
    for epoch in range(FL_params.global_epoch):
        ...
        ## confusing naming here? current_global model assigned as newGM_t, current_client_model as oldCM_t
        current_global_model = uncali_global_models[epoch] #newGM_t
        ## Both of these are in the inputs
        ...
        for layer in current_global_model.state_dict().keys():
            ...
            old_param_update[layer] = 0*current_global_model.state_dict()[layer] ## (oldCM_t - oldGM_t)
            return_model_state[layer] = 0*current_global_model.state_dict()[layer] ## newGM_t+1
            
            for ii in range(len(current_client_models)):
                old_param_update[layer] += current_client_models[ii].state_dict()[layer]
            
            ## Division here is to normalise (multiple client models)
        ...
        ## return_model_state = dict() is newGM_t+1
